-- FORCE DROP AND RECREATE with schema qualification
DROP FUNCTION IF EXISTS public.assign_credentials_to_pending_orders(uuid) CASCADE;
DROP FUNCTION IF EXISTS assign_credentials_to_pending_orders(uuid) CASCADE;
DROP FUNCTION IF EXISTS assign_credentials_to_pending_orders() CASCADE;

-- Recreate with explicit NULL handling using IS NOT DISTINCT FROM
...
BEGIN
  ...
  LOOP
    ...
    -- This handles NULL = NULL as TRUE, and requires exact UUID match for non-NULL
    ...
    WHERE pc.product_id = v_order_item.product_id
      AND pc.plan_id IS NOT DISTINCT FROM v_order_item.plan_id  -- CRITICAL LINE!
      AND (
        SELECT COUNT(*) 
        FROM account_seats 
