-- Force recreate the function with strict matching
...
BEGIN
  ...
  LOOP
    ...
    -- STRICT MATCHING: plan_id must match EXACTLY - no fallbacks!
    ...
    WHERE pc.product_id = v_order_item.product_id
      -- CRITICAL: Both must be NULL or both must match exactly
      AND pc.plan_id = v_order_item.plan_id  -- STRICT: exact match only, no OR clause!
      AND (
        SELECT COUNT(*) 
        FROM account_seats 
    ...
    IF FOUND THEN
      ...
      END IF;
    ELSE
      RAISE NOTICE 'NO MATCH FOUND: No credential with plan_id=% for product=% - order item % remains pending', 
        v_order_item.plan_id, v_order_item.product_id, v_order_item.order_item_id;
    END IF;
  END LOOP;
