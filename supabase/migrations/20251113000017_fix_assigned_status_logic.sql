-- Fix credential assignment to only mark as assigned when ALL seats are successful
...
BEGIN
  ...
  LOOP
    ...
    IF FOUND THEN
      ...
      -- Check if credential should be marked as assigned
      -- Only mark as assigned if ALL seats are filled AND all are successful
      SELECT 
        COUNT(*) >= v_credential.max_seats INTO v_seats_filled
      FROM account_seats 
      WHERE credential_id = v_credential.id;
      
      IF v_seats_filled THEN
        SELECT NOT EXISTS (
          SELECT 1 FROM account_seats 
          WHERE credential_id = v_credential.id 
          AND status != 'success'
        ) INTO v_all_success;
        ...
        IF v_all_success THEN
          ...
          RAISE NOTICE 'Credential % marked as FULLY ASSIGNED (all seats successful)', v_credential.id;
        ELSE
          RAISE NOTICE 'Credential % seats filled but NOT all successful yet', v_credential.id;
        END IF;
      END IF;
