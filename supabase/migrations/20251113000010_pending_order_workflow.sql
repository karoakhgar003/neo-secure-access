-- Update credential assignment function to support pending order workflow
-- Orders start as 'pending' and are marked 'completed' only when all items have credentials
...
BEGIN
  ...
  FOR v_order_item IN
    ...
    WHERE (oi.status = 'pending' OR oi.status IS NULL)
      ...
      AND o.status = 'pending'  -- Process orders in pending status
    ORDER BY oi.created_at ASC
  LOOP
    -- Find an available credential for this product AND plan
    -- STRICT MATCHING: Only assign if plan_id matches exactly
    ...
    WHERE pc.product_id = v_order_item.product_id
      -- STRICT: plan_id must match exactly (both specific or both NULL)
      AND (
    ...
    IF FOUND THEN
      ...
      END IF;
    ELSE
      RAISE NOTICE 'No available credential for product % with plan % - order item remains pending', v_order_item.product_id, v_order_item.plan_id;
    END IF;
  END LOOP;
END;
$$ LANGUAGE plpgsql;
